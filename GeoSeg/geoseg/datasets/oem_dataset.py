...
class OEMTrainDataset(Dataset):
    def __init__(
        self,
        data_root="OpenEarthMap/",
        paths="train.txt",
        mosaic_ratio=0.25,
        img_suffix=".png",
        mask_suffix=".png",
        ...
        self.paths = paths
        self.img_ids = self.get_img_ids(self.data_root, self.paths)
    ...
    def get_img_ids(self, data_root, paths):
        img_pths = [f for f in Path(data_root).rglob("*.tif") if "/labels/" in str(f)]

        pths = [
            str(f)
            for f in img_pths
            if f.name in np.loadtxt(data_root + paths, dtype=str)
        ]
    ...
    def load_img_and_mask(self, index):
        path = self.img_ids[index]
        mask = Image.open(path).convert("L")
        img = Image.open(path.replace("/labels/", "/images/")).convert("RGB")
...
oem_val_dataset = OEMTrainDataset(
    data_root="OpenEarthMap/", paths="val.txt", mosaic_ratio=0.0, transform=val_aug
)


class OEMTestDataset(Dataset):
    def __init__(
        self,
        data_root="OpenEarthMap/",
        img_suffix=".png",
    ...
    def get_img_ids(self, data_root):
        img_pths = [f for f in Path(data_root).rglob("*.tif") if "/labels/" in str(f)]

        pths = [
            str(f)
            for f in img_pths
            if f.name in np.loadtxt(data_root + "train.txt", dtype=str)
        ]
    ...
    def load_img(self, index):
        img_id = self.img_ids[index]
        img = Image.open(img_id.replace("/labels/", "/images/")).convert("RGB")
